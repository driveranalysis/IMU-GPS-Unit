...
#define FILE_BASE_NAME "mpu9250a" // setting the filename
struct data_t {
  unsigned long time;
  int16_t ax;
  int16_t ay;
...
void acquireData(data_t* data) {
  ...
  data->time = micros();
  mpu.update(); 
  data->ax =  mpu.getAcc(1);
  data->ay =  mpu.getAcc(2);
  data->az =  mpu.getAcc(3);
  data->gx =  mpu.getGyro(1);
  data->gy =  mpu.getGyro(2);
  data->gz =  mpu.getGyro(3);
//  uint8_t a = 1;
//  uint8_t b = 2;
//  uint8_t c = 3;
  data->mx =  mpu.getMag(1);
  data->my =  mpu.getMag(2);
  data->mz =  mpu.getMag(3);
}
...
void createBinFile() {
  ...
  if (sd.exists(TMP_FILE_NAME)) {
    ...
    //sd.remove(TMP_FILE_NAME); //replacement
  ...
  Serial.println(F("\nCreating new file"));
  binFile.close();
//  binFile.createContiguous(TMP_FILE_NAME, 512 * FILE_BLOCK_COUNT); //replacement
  if (!binFile.createContiguous(TMP_FILE_NAME, 512 * FILE_BLOCK_COUNT)) {
    error("createContiguous failed");
  }
  // Get the address of the file on the SD.
//  binFile.contiguousRange(&bgnBlock, &endBlock); //replacement
...
//    sd.card()->erase(bgnErase, endErase); //replacement
...
void recordBinFile() {
  ...
  //sd.card()->writeStart(binFile.firstBlock()); //replacement
...
//      sd.card()->writeData((uint8_t*)pBlock); //replacement
...
//    binFile.truncate(512L * bn); //replacement
...
//  binFile.truncate(512 * (bgnBlock + 1)); //replacement
